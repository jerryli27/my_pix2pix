...
parser.add_argument("--gen_train_prob", type=float, default=0.2,
                    help="The probability of training only the discriminator.")
...
def create_model(inputs, targets, is_hint_off=None):
    def create_generator(generator_inputs, generator_outputs_channels, trainable = True):
        ...
        layer_specs = [
            ...
            a.ngf * 16,  # encoder_10: [batch, h/16, w/16, ngf * 16] => [batch, h/32, w/32, ngf * 16]
            a.ngf * 16,  # encoder_11: [batch, h/32, w/32, ngf * 16] => [batch, h/32, w/32, ngf * 16]
            a.ngf * 16,  # encoder_12: [batch, h/32, w/32, ngf * 16] => [batch, h/64, w/64, ngf * 16]
            a.ngf * 16,  # encoder_13: [batch, h/64, w/64, ngf * 16] => [batch, h/64, w/64, ngf * 16]
        ]
        ...
        # layer_specs = [
        #     a.ngf * 16,  # encoder_10: [batch, h/16, w/16, ngf * 16] => [batch, h/32, w/32, ngf * 16]
        #     a.ngf * 16,  # encoder_11: [batch, h/32, w/32, ngf * 16] => [batch, h/64, w/64, ngf * 16]
        #     a.ngf * 16,  # encoder_12: [batch, h/64, w/64, ngf * 16] => [batch, h/128, w/128, ngf * 16]
        #     a.ngf * 16,  # encoder_13: [batch, h/128, w/128, ngf * 16] => [batch, h/128, w/128, ngf * 16]
        # ]
        #
        # for out_channels_i, out_channels in enumerate(layer_specs):
        #     with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
        #         if out_channels_i != len(layer_specs) -1:
        #             # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
        #             convolved = conv(layers[-1], out_channels, stride=2, shift=4, trainable=trainable)
        #         else:
        #             # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
        ...
        #
        # layer_specs = [
        #     (a.ngf * 16),   # decoder_8: [batch, h/128, w/128, ngf * 16 * 2]=> [batch, h/64, w/64, ngf * 16]
        #     (a.ngf * 16),   # decoder_7: [batch, h/64, w/64, ngf * 16 * 2] => [batch, h/32, w/32, ngf * 16]
        #     (a.ngf * 16),   # decoder_6: [batch, h/32, w/32, ngf * 16 * 2] => [batch, h/16, w/16, ngf * 16]
        # ]
        ...
        layer_specs = [
            (a.ngf * 16),  # decoder_12: [batch, h/64, w/64, ngf * 16 * 2]=> [batch, h/32, w/32, ngf * 16]
            (a.ngf * 16),  # decoder_11: [batch, h/32, w/32, ngf * 16]=> [batch, h/32, w/32, ngf * 16]
            (a.ngf * 16),  # decoder_10: [batch, h/32, w/32, ngf * 16 * 2] => [batch, h/16, w/16, ngf * 16]
            (a.ngf * 16),  # decoder_9: [batch, h/16, w/16, ngf * 16] => [batch, h/16, w/16, ngf * 16]
    ...
    def create_discriminator(discrim_inputs, discrim_targets):
        ...
        layer_specs = [
            ...
            a.ndf * 8,  # encoder_7: [batch, h/8, w/8, ngf * 8] => [batch, h/16, w/16, ngf * 8]
            a.ndf * 16,  # encoder_8: [batch, h/16, w/16, ngf * 16] => [batch, h/16, w/16, ngf * 16]
        ]
        for out_channels in layer_specs:
        ...
        # layer_5: [batch, h/16, w/16, ngf * 16] => [batch, h/16, w/16, 1]
        with tf.variable_scope("layer_%d" % (len(layers) + 1)):
    ...
    if a.use_sketch_loss:
        update_losses = ema.apply([discrim_loss, gen_loss_GAN_no_prob, gen_loss_L1, gen_loss_sketch])
    else:
        update_losses = ema.apply([discrim_loss, gen_loss_GAN_no_prob, gen_loss_L1])
    ...
    return Model(
        ...
        gen_loss_GAN=ema.average(gen_loss_GAN_no_prob),
