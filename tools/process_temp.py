"""
This file is for combining files when the images in b_dir are all in the same directory instead of each having its
own subdirectory like that in the input_dir. input_dir contains bw or sketch image where b_dir contains colored ones.
"""
...
def main():
    ...
    with tf.Session(config=config) as sess:
        for num_image, src_path in enumerate(find(a.b_dir)):
            ...
            if a.operation == "combine":
                ...
                sibling_path = os.path.join(a.input_dir, os.path.basename(src_path))
                if os.path.exists(sibling_path):
                    sibling = load(sibling_path)
                else:
                    raise Exception("could not find sibling image for " + src_path +
                                    ". Sibling path: " + sibling_path +" does not exist under bdir=" + a.b_dir)
                ...
                dst = np.concatenate([sibling, src], axis=1)
            else:
