...
# This version implements https://arxiv.org/pdf/1704.00028.pdf
...
parser.add_argument("--dilation_sketch_max_width", type=int, default=6,
                    help="The probability of having sketch generated by dilation as the input instead of using the "
                         "provided input sketches.")
parser.add_argument("--l1_weight_decay_lower_threshold", type=int, default=50,
                    help="The probability of having sketch generated by dilation as the input instead of using the "
                         "provided input sketches.")
...
L1_WEIGHT_DECAY_LOWER_THRESHOLD=a.l1_weight_decay_lower_threshold
...
def create_model(inputs, targets, is_hint_off=None):
    ...
    def create_discriminator(input):
        ...
        with tf.variable_scope("layer_1"):
            ...
            # TODO: maybe replace batch norm with instance norm in the discriminator. Don't forget below, in for loop.
            #normed = convolved
    ...
    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator" if not a.train_sketch else SKETCH_VAR_SCOPE_PREFIX + "discriminator"):
            if a.use_hint:
                ...
                predict_real = create_discriminator(input = tf.concat(3, [inputs[...,:1], targets]))
            else:
                predict_real = create_discriminator(input = tf.concat(3, [inputs, targets]))

    with tf.name_scope("fake_discriminator"):
        with tf.variable_scope("discriminator" if not a.train_sketch else SKETCH_VAR_SCOPE_PREFIX + "discriminator", reuse=True):
            if a.use_hint:
                print(inputs[...,:1].get_shape().as_list())
                predict_fake = create_discriminator(input = tf.concat(3, [inputs[...,:1], outputs]))
            else:
                predict_fake = create_discriminator(input = tf.concat(3, [inputs, outputs]))
    ...
    with tf.name_scope("discriminator_loss"):
        ...
        # Taken from https://github.com/igul222/improved_wgan_training/blob/master/gan_toy.py
        # WGAN gradient penalty

        alpha = tf.random_uniform(
            shape=[a.batch_size, 1, 1, 1],
            minval=0.,
            maxval=1.
        )
        if a.use_hint:
            real_data = tf.concat(3, [inputs[...,:1], targets])
            fake_data = tf.concat(3, [inputs[...,:1], outputs])
        else:
            real_data = tf.concat(3, [inputs, targets])
            fake_data = tf.concat(3, [inputs, outputs])
        interpolates = alpha * real_data + ((1 - alpha) * fake_data)
        with tf.name_scope("interpolation_discriminator"):
            with tf.variable_scope("discriminator" if not a.train_sketch else SKETCH_VAR_SCOPE_PREFIX + "discriminator",
                                   reuse=True):
                disc_interpolates = create_discriminator(interpolates)
    ...
    with tf.name_scope("discriminator_train"):
        ...
        discrim_train = slim.learning.create_train_op(discrim_loss, tf.train.AdamOptimizer(a.lr, beta1=0.5, beta2=0.9),
                                                    variables_to_train=discrim_tvars)
    ...
    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            ...
            gen_train = slim.learning.create_train_op(gen_loss, tf.train.AdamOptimizer(a.lr, beta1=0.5, beta2=0.9),
                                                      variables_to_train=gen_tvars)
