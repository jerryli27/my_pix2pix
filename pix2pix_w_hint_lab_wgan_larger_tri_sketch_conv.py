...
# -*- coding: utf-8 -*-
"""
Not only has extra parameters in instance norm but conv weights as well.
"""
...
if a.mode != "test" and a.single_input:
    parser.error("single input mode is intended for test mode where only output is needed.")
...
def one_hot_conv(batch_input, out_channels, stride, one_hot_style_vector, shift=4, pad = 1, trainable=True):
    with tf.variable_scope("conv"):
        in_channels = batch_input.get_shape().as_list()[3]

        if one_hot_style_vector is None:
            var_shape = [shift, shift, in_channels, out_channels]
        else:
            one_hot_style_vector = tf.identity(one_hot_style_vector)
            num_styles = one_hot_style_vector.get_shape().as_list()[1]
            # num_styles = one_hot_style_vector.shape[1]
            var_shape = [num_styles, shift, shift, in_channels, out_channels]

        filter = tf.get_variable("filter", var_shape, dtype=tf.float32,
                                 initializer=tf.random_normal_initializer(0, 0.02), trainable=trainable)

        if one_hot_style_vector is not None:
            filter = tf.reshape(filter, [num_styles, -1])
            filter = tf.matmul(one_hot_style_vector, filter)
            assert filter.get_shape().as_list()[0] == 1
            filter = tf.reshape(filter[0], var_shape[1:])
...
def smart_conv(batch_input, out_channels, stride, shift=4, pad = 1, one_hot_style_vector=None, trainable=True):
    if one_hot_style_vector is None:
        return conv(batch_input, out_channels, stride, shift=shift, pad = pad, trainable=trainable)
    else:
        return one_hot_conv(batch_input, out_channels, stride, one_hot_style_vector, shift=shift, pad = pad, trainable=trainable)
...
def one_hot_deconv(batch_input, out_channels,one_hot_style_vector, stride = 2, shift = 4, trainable=True):
    with tf.variable_scope("deconv"):
        batch, in_height, in_width, in_channels = [int(d) for d in batch_input.get_shape()]

        if one_hot_style_vector is None:
            var_shape = [shift, shift, out_channels, in_channels]
        else:
            one_hot_style_vector = tf.identity(one_hot_style_vector)
            num_styles = one_hot_style_vector.get_shape().as_list()[1]
            # num_styles = one_hot_style_vector.shape[1]
            var_shape = [num_styles, shift, shift, out_channels, in_channels]


        filter = tf.get_variable("filter", var_shape, dtype=tf.float32, initializer=tf.random_normal_initializer(0, 0.02), trainable=trainable)
        if one_hot_style_vector is not None:
...
def smart_deconv(batch_input, out_channels, stride = 2, shift=4, one_hot_style_vector=None, trainable=True):
    if one_hot_style_vector is None:
        return deconv(batch_input, out_channels, stride, shift=shift, trainable=trainable)
    else:
        return one_hot_deconv(batch_input, out_channels, one_hot_style_vector, stride, shift=shift, trainable=trainable)
...
def create_model(inputs, targets):
    def create_generator(generator_inputs, generator_outputs_channels, one_hot_vec=None, trainable = True):
        ...
        with tf.variable_scope("encoder_1"):
            ...
            convolved = smart_conv(generator_inputs, a.ngf, stride=1, shift=3, one_hot_style_vector=one_hot_vec, trainable=trainable)
        ...
        for out_channels in layer_specs:
            with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
                ...
                if (len(layers) + 1) % 2 == 0:
                    ...
                    convolved = smart_conv(layers[-1], out_channels, stride=2, shift=4, one_hot_style_vector=one_hot_vec, trainable=trainable)
                else:
                    # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
                    convolved = smart_conv(layers[-1], out_channels, stride=1, shift=3, one_hot_style_vector=one_hot_vec, trainable=trainable)
        ...
        for decoder_layer, (out_channels) in enumerate(layer_specs):
            ...
            with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                if decoder_layer % 2 != 0:
                    ...
                    output = smart_deconv(input, out_channels, 1, 3, one_hot_style_vector=one_hot_vec, trainable=trainable)
                else:
                    ...
                    # [batch, in_height, in_width, in_channels] => [batch, in_height*2, in_width*2, out_channels]
                    output = smart_deconv(input, out_channels, 2, 4, one_hot_style_vector=one_hot_vec, trainable=trainable)
                # if decoder_layer == 0:
                #     # first decoder layer doesn't have skip connections
                #     # since it is directly connected to the skip_layer
                #     input = layers[-1]
        ...
        with tf.variable_scope("decoder_1"):
            ...
            output = smart_deconv(input, generator_outputs_channels, 1, 3, one_hot_style_vector=one_hot_vec, trainable=trainable)
    ...
    if a.from_128:
        ...
        with tf.Session(config=config) as sess:
            ...
            if checkpoint is None:
                raise IOError("Checkpoint %s does not exist!" %(a.checkpoint))
        ...
        if a.use_sketch_loss and a.checkpoint is None:
            ...
            with tf.Session(config=config) as sess:
                ...
                if pretrained_sketch_checkpoint is None:
                    raise IOError("Checkpoint %s does not exist!" % (a.pretrained_sketch_net_path))
