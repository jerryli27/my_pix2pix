...
def create_model(inputs, targets):
    def create_generator(generator_inputs, generator_outputs_channels, trainable = True):
        ...
        # Dilation layers
        dilation_layer_specs = [
            a.ngf * 8,
            a.ngf * 8,
            a.ngf * 8,
        ]
        for dilation_i, out_channels in enumerate(dilation_layer_specs):
            with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
                rectified = lrelu(layers[-1], 0.2)
                # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
                in_height, in_width = rectified.get_shape().as_list()[1:3]
                convolved = conv(rectified, out_channels,stride=None, dilation=2**(dilation_i+1), trainable=trainable)
                output = batchnorm(convolved)
                layers.append(output)

        layer_specs = [
            (a.ngf * 4, 0.0),   # decoder_4: [batch, 16, 16, ngf * 8] => [batch, 32, 32, ngf * 4 * 2]
        ...
        for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
            ...
            skip_layer = num_encoder_layers - decoder_layer - 1 - len(dilation_layer_specs)
            with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                if decoder_layer == 0:
                    # first decoder layer doesn't have skip connections
                    # since it is directly connected to the skip_layer
                    # input = layers[-1]
                    input = tf.concat(3, [layers[-dilation_i] for dilation_i in range(1,len(dilation_layer_specs)+2)]) # Combine the dilation layers along with the last encode layer.
                else:
...
"""
"""
python pix2pix_w_hint_better_gen.py --mode train --output_dir pixiv_full_128_wgan_sketch_loss_better_gen --max_epochs 20 --input_dir /mnt/tf_drive/home/ubuntu/pixiv_full_128_combined/train --which_direction AtoB --display_freq=1000 --gray_input_a --batch_size 4 --lr 0.0008 --gpu_percentage 0.45 --scale_size=143 --crop_size=128 --use_sketch_loss --pretrained_sketch_net_path pixiv_full_128_to_sketch_train --use_hint
"""
