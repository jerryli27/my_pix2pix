...
def load_examples(user_hint = None):
    ...
    with tf.name_scope("load_images"):
        ...
        if a.noise:
            ...
            b_images_random_noise = tf.random_normal([1],mean=0.0,stddev=0.04, name="a_images_random_noise")
            b_images = b_images + b_images_random_noise
            b_images = tf.clip_by_value(b_images, 0 , 1)

            a_images_random_noise = tf.random_normal([1], mean=0.0, stddev=0.04, name="a_images_random_noise_1")\
                                    + tf.random_normal([1], mean=0.0, stddev=0.12, name="a_images_random_noise_2")
