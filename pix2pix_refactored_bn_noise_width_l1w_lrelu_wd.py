...
parser.add_argument("--dilation_sketch_max_width", type=int, default=6,
                    help="The probability of having sketch generated by dilation as the input instead of using the "
                         "provided input sketches.")
parser.add_argument("--l2_regularizer_term", type=float, default=0.0005,
                    help=" ")
...
# In the original paintschainer code the weight decay was 0.00001. TODO: test serveral values.
L2_REGULARIZER_TERM = a.l2_regularizer_term
...
def slim_conv(batch_input, out_channels, stride, shift=4, pad = 1, trainable=True):
    with tf.variable_scope("conv"):
        ...
        # conv = tf.nn.conv2d(padded_input, filter, [1, stride, stride, 1], padding="VALID")
        # return conv
        conv = slim.conv2d(batch_input, out_channels, [shift, shift], stride=stride, padding='SAME', weights_regularizer=slim.l2_regularizer(L2_REGULARIZER_TERM), trainable=trainable)
        return conv
...
def slim_deconv(batch_input, out_channels, stride = 2, shift = 4, trainable=True):
    with tf.variable_scope("deconv"):
        ...
        conv = slim.conv2d_transpose(batch_input, out_channels, [shift, shift], stride=stride, padding='SAME', weights_regularizer=slim.l2_regularizer(L2_REGULARIZER_TERM), trainable=trainable)
        return conv
...
def load_examples(user_hint = None):
    ...
    with tf.name_scope("load_images"):
        ...
        if a.noise:
            ...
            b_images_random_noise = tf.random_normal([1],mean=0.0,stddev=0.04, name="b_images_random_noise")
