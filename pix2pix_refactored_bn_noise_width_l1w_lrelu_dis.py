...
# This version does not use the PatchGAN proposed by pix2pix, but uses a normal discriminator instead,
...
def create_model(inputs, targets, is_hint_off=None):
    ...
    def create_discriminator(discrim_inputs, discrim_targets):
        ...
        # # layer_5: [batch, h/8, h/8, ndf * 8] => [batch, 1, 1, 1]
        with tf.variable_scope("fc_%d" % (len(layers) + 1)):
            # With WGAN, sigmoid for the last layer is no longer needed
            shape = rectified.get_shape().as_list()
            assert shape[1] == shape[2]
            # flattened_dim = np.prod(shape[1:])
            # flattened = tf.reshape(rectified, [shape[0],1,1,flattened_dim])
            # convolved = slim.fully_connected(flattened, 1, activation_fn=None)
            convolved = conv(rectified, out_channels=1, stride=shape[1], shift=1, pad=0)

            assert convolved.get_shape().as_list() == [shape[0],1,1,1]
            layers.append(convolved)
